...
public:
	...
	void ProjectOntoCylinder(cv::Mat InitialImage, cv::Mat& TransformedImage, std::vector<int>& mask_x, std::vector<int>& mask_y, int f)
	{
		int h = InitialImage.rows, w = InitialImage.cols;
		int center_x = w / 2, center_y = h / 2;
		// int f = 1000;			// 1100 field; 1000 Sun; 1500 Rainier; 1050 Helens
